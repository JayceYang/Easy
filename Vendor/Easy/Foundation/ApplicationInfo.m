...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    self.currencyCode = [aDecoder decodeObjectForKey:@"currencyCode"];
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    value.currencyCode = [self.currencyCode copyWithZone:zone];
    value.currencySymbol = [self.currencySymbol copyWithZone:zone];
    return value;
}
...
+ (instancetype)sharedInfo
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:[userInfo filePath]]) {
            ...
            ELog(@"Make UUID %@ first time.", UUID);
            userInfo.UUID = UUID;
            [userInfo clear];
...
//    }
    ...
    self.currencyCode = [NSString string];
    self.currencySymbol = self.currencyCode;
    [self write];
}
